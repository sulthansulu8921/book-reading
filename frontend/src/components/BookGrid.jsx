...
export default function BookGrid() {
    ...
    return (
        <div className="min-h-screen bg-[#0a0f18] text-slate-200 selection:bg-blue-500/30">
            {/* Hero Section */}
            <div className="relative overflow-hidden pt-20 pb-8 px-6 md:px-12 lg:px-16 border-b border-white/5">
                ...
                <div className="max-w-7xl mx-auto relative z-10">
                    ...
                    >
                        ...
                        <div className="flex bg-white/5 backdrop-blur-xl border border-white/10 p-1 rounded-2xl overflow-x-auto no-scrollbar max-w-full">
                            {categories.map((cat) => (
                                <button
                                    key={cat}
                                    onClick={() => setFilter(cat)}
                                    className={`px-4 md:px-6 py-2 rounded-xl text-xs md:text-sm font-semibold transition-all duration-300 whitespace-nowrap ${filter === cat
            ...
            {recentBooks.length > 0 && filter === "All" && (
                <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16 pt-12 md:pt-16">
                    <div className="flex items-center gap-3 mb-6 md:mb-8">
                        <div className="w-1 h-5 md:w-1.5 md:h-6 bg-blue-600 rounded-full" />
                        <h2 className="text-lg md:text-xl font-black uppercase tracking-[0.2em] text-white">Continue Reading</h2>
                    </div>
                    ...
                    <div className="flex gap-4 md:gap-6 overflow-x-auto pb-8 snap-x no-scrollbar">
                        {recentBooks.map((book) => (
                            <motion.div
                                ...
                                className="flex-none w-64 md:w-72 h-40 md:h-44 group cursor-pointer snap-start relative rounded-2xl md:rounded-3xl overflow-hidden bg-[#151b27] border border-white/5 shadow-2xl"
                            >
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-4 md:p-6 flex flex-col justify-end">
                                    <h3 className="text-white font-black text-base md:text-lg truncate mb-1 md:mb-2">{book.title}</h3>
                                    <div className="flex items-center justify-between gap-3">
                                        ...
                                        <span className="text-[8px] md:text-[10px] font-bold text-blue-400 uppercase whitespace-nowrap">Page {book.lastPage + 1}</span>
                                    </div>
                                </div>
                            </motion.div>
                        ))}
            ...
            <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16 py-12 md:py-16">
                <div className="flex items-center gap-3 mb-8 md:mb-10">
                    <div className="w-1 h-5 md:w-1.5 md:h-6 bg-slate-600/30 rounded-full" />
                    <h2 className="text-lg md:text-xl font-black uppercase tracking-[0.2em] text-white/50">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-10">
                    <AnimatePresence mode="popLayout">
                        {filteredBooks.map((book, idx) => (
                            ...
                            >
                                <div className="relative aspect-[2/3] rounded-xl md:rounded-2xl overflow-hidden bg-[#151b27] shadow-[0_10px_30px_-10px_rgba(0,0,0,0.5)] transition-all duration-500 group-hover:shadow-[0_20px_40px_-15px_rgba(59,130,246,0.3)]">
                                    <BookCover title={book.title} coverUrl={book.cover} />

                                    {/* Glass Overlay on Hover (Desktop) / Visible elements for touch */}
                                    <div className="absolute inset-0 bg-blue-600/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col items-center justify-center z-20">
                                        <div className="p-2 md:p-3 rounded-xl md:rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 scale-90 group-hover:scale-100 transition-transform duration-500 shadow-xl">
                                            <BookOpen className="text-white" size={20} />
                                        </div>
                                        <span className="mt-4 text-white text-[8px] md:text-[10px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                                ...
                                {/* Glow Effect */}
                                <div className="absolute -inset-1 bg-gradient-to-r from-blue-600/20 to-indigo-600/20 rounded-[18px] md:rounded-[22px] blur-xl opacity-0 group-hover:opacity-100 transition duration-700 -z-10" />
                            </motion.div>
                        ))}
                    </AnimatePresence>
                </div>
